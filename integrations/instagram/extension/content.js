 ...
 */
export async function updateDocId({ type, docId }) {
  try {
...
/**
 * Register message handlers with the core extension
 * Called by core content.js when on Instagram
 */
export function registerHandlers(registerHandler) {
  registerHandler(MSG.SCRAPE_INSTAGRAM, () => scrapeProfile());
  registerHandler(MSG.FETCH_INSTAGRAM_PROFILE, (payload) => fetchProfileAPI(payload));
  registerHandler(MSG.UPDATE_DOC_ID, (payload) => updateDocId(payload));
}
